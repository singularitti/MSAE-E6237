...
\subsection{Rahman and Parrinello's approach}
...
\begin{equation}
	...
	\frac{ 1 }{ 2 } W \Tr (\dot{h} \tran \dot{h}) - P \Omega,
\end{equation}
...
\begin{align}\label{eq:rplag}
	\ddot{s}_i & = \frac{ 1 }{ m_i } \sum_{j\neq i}
	\frac{ \phi'(r_{ij}) }{ r_{ij} } (\bm{s}_i - \bm{s}_j) - g^{-1}\dot{g}
	\dot{ \bm{s} }_i, \quad i, j = 1, 2, \ldots, N,\\
	\ddot{h}   & = \frac{ 1 }{ W } (\Pi - P) \sigma, \label{eq:hdd}
\end{align}
where $\sigma = \{\bm{b}\times \bm{c}, \bm{c}\times \bm{a},
\bm{a}\times \bm{b}\}$, matrix $\Pi$ is given by
\begin{equation}
	\Omega \Pi = \sum_{i=1}^{N} m_i \bm{v}_i \otimes \bm{v}_i
	+ \sum_{i=1}^{N} \sum_{i<j} \frac{ \phi'(r_{ij}) }{ r_{ij} }
	(\bm{r}_i - \bm{r}_j)\otimes (\bm{r}_i - \bm{r}_j),
\end{equation}
...
\begin{equation}
	\mathcal{L} = \frac{ 1 }{ 2 } \sum_{i=1}^{N} m_i \dot{ \bm{s} }_i \tran
	g \dot{ \bm{s} }_i - \sum_{i=1}^{N} \sum_{i<j} \phi (r_{ij}) +
	\frac{ 1 }{ 2 } W \Tr (\dot{h} f_0 \dot{h}\tran) - P \Omega,
\end{equation}
